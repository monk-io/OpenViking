...
> **Memory schema 作用域**：Memory schema YAML 默认 `stage: "user"` 且 `enable_peer: true`。trajectories/experiences 这类执行派生 schema 使用 `stage: "agent"`；如果某类记忆必须保留在当前用户目录而不是 peer 目录，设置 `enable_peer: false`。
