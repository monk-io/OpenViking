...
import { AUTO_RECALL_SOURCE_MARKER, buildAutoRecallContext, prepareRecallQuery, } from "./auto-recall.js";
import { compileSessionPatterns, extractNewTurnMessages, shouldBypassSession, } from "./text-utils.js";
...
export function toRoleId(senderId) {
    if (!senderId) {
        return undefined;
    }
...
function totalExtractedMemories(memories) {
...
const OPENVIKING_OV_SESSION_UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
 ...
 */
export function openClawSessionToOvStorageId(sessionId, sessionKey) {
...
export function openClawSessionRefToOvStorageId(ref) {
    const t = ref.trim();
    if (!t) {
 ...
 */
export function convertToAgentMessages(msg) {
...
function canonicalizeAssistantBlock(block) {
    if (!block || typeof block !== "object") {
        return block;
    }
    const rec = block;
...
function canonicalizeAgentMessages(messages) {
    let changed = false;
...
export function formatMessageFaithful(msg) {
    ...
    for (const part of msg.parts) {
        ...
        switch (part.type) {
            ...
            case "context":
                sections.push(`[Context: ${part.uri ?? "?"}]${part.abstract ? ` ${part.abstract}` : ""}`);
                break;
            default:
...
function buildArchiveMemory(archiveOverview, _preAbstracts, _budget) {
    const messages = [];
    if (archiveOverview) {
        messages.push({
            role: "user",
 ...
 * Tracks issue #1724.
 */
export function mergeConsecutiveUsers(messages) {
    const result = [];
    for (const msg of messages) {
        const prev = result[result.length - 1];
        if (msg.role === "user" && prev?.role === "user") {
            const prevContent = Array.isArray(prev.content) ? prev.content : [{ type: "text", text: prev.content }];
            const currContent = Array.isArray(msg.content) ? msg.content : [{ type: "text", text: msg.content }];
            prev.content = hoistToolResults([...prevContent, ...currContent]);
        }
        else {
            result.push({ ...msg });
        }
 ...
 */
export function ensureAlternation(messages) {
    const result = [];
    for (const msg of messages) {
        const prev = result[result.length - 1];
...
function buildSessionContext(ovMessages, budget) {
 ...
 */
async function pollPhase2ExtractionOutcome(getClient, taskId, agentId, logger, sessionLabel) {
...
export function createMemoryOpenVikingContextEngine(params) {
    const { id, name, version, cfg, logger, getClient, resolveAgentId, rememberSessionAgentId, } = params;
    ...
    function assemblePassthrough(ovSessionId, reason, liveMessages, originalTokens, extra) {
    ...
    function buildAssembledContext(overview, preAbstracts, ovMessages, tokenBudget, ovSessionId) {
        ...
        const sanitized = ensureAlternation(mergeConsecutiveUsers(sanitizeToolUseResultPairing(canonical)));
    ...
    return {
        ...
        async assemble(assembleParams) {
            ...
            try {
                ...
                const { sanitized, archive, session, budgets, instruction } = buildAssembledContext(ctx.latest_archive_overview, preAbstracts, ctx.messages, tokenBudget, OVSessionId);
        ...
        async afterTurn(afterTurnParams) {
            ...
            try {
                ...
                const routingRef = afterTurnParams.sessionId ?? sessionKey ?? OVSessionId;
                const agentId = resolveAgentId(routingRef, sessionKey, OVSessionId);
                ...
                const start = typeof afterTurnParams.prePromptMessageCount === "number" &&
                ...
                for (const msg of extractedMessages) {
                    ...
                    if (ovParts.length > 0) {
                        await client.addSessionMessage(OVSessionId, msg.role, // 统一是 "user"
                        ovParts, agentId, createdAt, msg.role === "user" ? senderRoleId : undefined);
                    }
                }
                ...
                if (commitResult.task_id && cfg.logFindRequests) {
                    ...
                    void pollPhase2ExtractionOutcome(getClient, commitResult.task_id, agentId, logger, OVSessionId);
                }
            }
            catch (err) {
        ...
        async compact(compactParams) {
            ...
            if (typeof tokensBeforeOriginal !== "number") {
                try {
                    ...
                    if (typeof preCtx.estimatedTokens === "number" &&
            ...
            try {
                ...
                try {
                    ...
                    if (typeof ctx.estimatedTokens === "number" &&
