...
export { estimateAgentMessageTokens, estimateAgentMessagesTokens, estimateSerializedTokens, estimateTextTokens, } from "./token-estimator.js";
function parseToolResultRef(value) {
...
const contextEnginePlugin = {
    ...
    register(api) {
        ...
        const formatOVSearchRows = (result) => {
        ...
        const formatOVSearchText = (query, uri, result) => {
            if ((result.total ?? 0) <= 0) {
        ...
        const searchOpenViking = async (input, agentId) => {
            ...
            return {
                content: [{ type: "text", text: formatOVSearchText(query, input.uri, result) }],
                details: {
                    action: "searched",
                    query,
                    uri: input.uri,
        ...
        api.registerTool((ctx) => ({
            ...
            async execute(_toolCallId, params) {
                ...
                const { lines: memoryLines } = await buildMemoryLinesWithBudget(memories, (uri) => recallClient.read(uri, session.agentId), {
                    ...
                    if (memoriesCount === 0) {
                        ...
                        return {
                            content: [
                                {
                                    type: "text",
                                    text: `Memory extraction completed for session ${sessionId}, but produced 0 memories. ` +
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Restore the full original content of a tool result that was externalized by OpenViking. " +
            ...
            parameters: Type.Object({
                tool_output_ref: Type.String({
                    description: "Exact OV URI from the preview, e.g. viking://session/<session_id>/tool-results/<tool_result_id>",
                }),
            ...
            async execute(_toolCallId, params) {
                ...
                try {
                    const client = await getClient();
                    const result = await client.readToolResult(session.ovSessionId, parsed.toolResultId, { offset, limit, includeMetadata: true }, session.agentId);
        ...
        api.registerTool((ctx) => ({
            ...
            description: "Search inside an externalized tool result for a keyword. " +
            ...
            async execute(_toolCallId, params) {
                ...
                const contextChars = Math.max(0, getOptionalInteger(params.context_chars ?? params.contextChars, 300));
                try {
                    const client = await getClient();
                    const result = await client.searchToolResult(session.ovSessionId, parsed.toolResultId, query, { limit, contextChars }, session.agentId);
                    const matches = result.matches ?? [];
                    const text = matches.length
                        ? [
                            `Found ${matches.length} match(es) for "${query}" in ${parsed.ref}:`,
                            "",
                            ...matches.map((match, index) => `## Match ${index + 1} (offset ${match.offset})\n${match.snippet}`),
                        ].join("\n")
        ...
        api.registerTool((ctx) => ({
            ...
            description: "List externalized tool results for the current session. " +
            ...
            async execute(_toolCallId, params) {
                ...
                try {
                    const client = await getClient();
                    const result = await client.listToolResults(session.ovSessionId, { toolName, limit }, session.agentId);
